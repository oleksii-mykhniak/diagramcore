...
Приклад workflow — `docs/ci-examples/diagrams.example.yml` у цьому
репозиторії (навмисно поза `.github/workflows/` — інакше GitHub Actions
запускав би цей приклад як реальний workflow на кожен push/PR у самому
DiagramCore, зайвим третім прогоном поряд з `ci.yml`/`deploy-web.yml`).
Він використовує лише команди `dc`, вже реалізовані в попередніх кроках
плану (`dc validate`, `dc render`, `dc context`), і не залежить від
жодної зовнішньої інфраструктури, окрім GitHub Actions.
...
1. Скопіюйте `docs/ci-examples/diagrams.example.yml` у
