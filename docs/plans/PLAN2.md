...
**AC:**
- [x] Unit: всі 3 фікстури парсяться; стиснений payload розпаковується; кожна евристика shape→type покрита тестом; id-санітизація зберігає зв'язність links.
- [x] Playwright (нова `drawio-import.spec.ts`): імпорт фікстури → вузли на канві у позиціях з файлу (±5px), `yaml-source` містить очікувані nodes/links, Problems — OK.
- [x] Імпортована діаграма редагується як звичайна (додати вузол, undo — працює).
- [x] Битий/чужий файл → людське повідомлення, застосунок не падає.
...
**AC:**
- [x] `grep -rn "style={{[^}]*#[0-9a-fA-F]" web/src` — порожній (усі кольори — токени).
- [x] Tour проходить end-to-end у новому UI (playwright).
- [x] Повний `npm run test:e2e` зелений; smoke-набір — в обох темах; `npm test`, `npm run build`, `go test ./...`, `go vet ./...`, `./dc validate examples/*.dc.yaml` зелені.
- [x] `.claude/skills/design/SKILL.md` актуальний (перелік токенів збігається з `theme.css`).

**Фаза 10 завершена.** Кроки 10.1–10.13 виконані; Mermaid-імпорт
свідомо не реалізований (descope, кандидат на майбутню фазу — див.
`docs/deviations.md`).
