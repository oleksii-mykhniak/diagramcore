...
test('selecting a flow and clicking next 3x highlights the first 3 edges on the React Flow canvas', async ({
  page,
}) => {
  await page.goto('/');
...
test('autoplay runs the flow to completion and stops on the React Flow canvas', async ({ page }) => {
  await page.goto('/');
  await page.getByTestId('file-input').setInputFiles(authSystemPath);
  await expect(page.getByTestId('diagram-svg')).toBeVisible();
  await page.getByTestId('canvas-toggle').click();
